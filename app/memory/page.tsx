    ...
    <div className="p-8">
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        <button
          ...
          className={`px-3 py-1 rounded-full text-sm ${
            categoryFilter === 'All' ? 'bg-zinc-100 text-zinc-900' : 'bg-zinc-800 text-zinc-400'
          }`}
        >
          All
        </button>
        {CATEGORY_OPTIONS.map(cat => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-sm ${
              categoryFilter === cat ? 'bg-zinc-100 text-zinc-900' : 'bg-zinc-800 text-zinc-400'
            }`}
          >
            {cat.split(' ')[0]}
          </button>
      ...
      {filteredMemories.length === 0 ? (
        <div className="bg-zinc-900 rounded-lg border border-zinc-800 p-12 text-center">
          ...
          <p className="text-sm text-zinc-500">Capture your first memory above</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-zinc-400 mb-2 whitespace-pre-wrap">{memory.content}</p>
                  {memory.why_it_matters && (
                    <p className="text-sm text-zinc-500 italic">💡 {memory.why_it_matters}</p>
                  )}
                  {memory.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {memory.tags.map(tag => (
                        <span key={tag} className="text-xs px-2 py-0.5 bg-zinc-800 rounded text-zinc-400">
                          #{tag}
                        </span>
                      ))}
                    </div>
